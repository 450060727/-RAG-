...
<template>
  <view class="page">
    <view class="card">
      ...
      <!-- 新密码输入 -->
      <input class="input" v-model="password" password placeholder="新密码（至少 6 位）" />
      <!-- 确认新密码 -->
      <input class="input" v-model="confirmPassword" password placeholder="确认新密码" />
      <!-- 确认重置按钮 -->
      <button class="btn" :disabled="loading" @click="submit">确 认 重 置</button>
...
async function submit() {
  ...
  try {
    await request({
      ...
      method: 'POST',
      data: { email: email.value, code: code.value, password: password.value },
      auth: false
    })
