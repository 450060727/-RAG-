/**
 * service/kb 模块的 QianwenChatClient 类/接口定义。
 */
...
/**
 * 千问（DashScope OpenAI 兼容模式）对话客户端。
 */
@Component
/**
 * QianwenChatClient 类。
...
public class QianwenChatClient implements ChatClient {
    ...
    private static final Logger log = LoggerFactory.getLogger(QianwenChatClient.class);
    ...
    public ChatCompletion chat(String systemPrompt,
        ...
        if (apiKey == null || apiKey.isBlank()) {
            throw new BizException("未配置千问对话 API Key");
        }
        if (baseUrl == null || baseUrl.isBlank()) {
            throw new BizException("未配置千问对话 baseUrl");
        }
        if (model == null || model.isBlank()) {
            throw new BizException("未配置千问对话模型");
        }
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            if (!response.isSuccessful()) {
                log.error("千问对话请求失败: status={}, body={}", response.code(), rawJson);
                throw new BizException("千问对话请求失败: HTTP " + response.code());
            }

            ChatResponse resp = JSON.parseObject(rawJson, ChatResponse.class);
            if (resp == null || resp.choices == null || resp.choices.isEmpty()
                    || resp.choices.get(0).message == null) {
                throw new BizException("千问对话返回格式异常");
            }
            ...
            log.error("千问对话调用异常", e);
            throw new BizException("千问对话调用异常: " + e.getMessage());
        }
    }
